...
simulate_data_monte_carlo <-
  function(def, n, iterations) {
    data <- tibble::tibble()
    
    for (i in 1:iterations) {
      generated_data <- genData(n, def) |>
...
```{r, create-data}
data <-
  simulate_data_monte_carlo(def, n, iterations = no_of_iterations)|>
  mutate(
...
data.frame(
  ...
  results_power = c(success_power(cov1), success_power(cov2), success_power(cov3), success_power(cov4), success_power(cov5))
) |>
  as.data.frame() |>
