...
```{r, tab.cap = "Summary of literature verification about strata selected parameters (Mrozikiewicz-Rakowska et. al., 2023)"}
# summary table
...
```{r, tab.cap = "Summary of proportion test for minimization randomization with equal weights"}
...
rand_data <-
  cbind(simnr = rep(
    unique(data$simnr),
    times = rep(1, times = length(unique(data$simnr))) * nrow(rand_data) /
      2
  ), rand_data) %>%
  mutate(simnr = as.factor(simnr))
```
...
```{r, fig.cap= "Summary average smd in each randomization methods", warning=FALSE, fig.width=9, fig.height=6}
# boxplot
cov_balance_data |>
...
```{r, fig.cap= "Summary smd in each randomization methods in each covariants", warning = FALSE, fig.width=9, fig.height=6}
# violin plot 
cov_balance_data |>
...
```{r, tab.cap = "Summary of percent success in each randomization methods"}
data.frame(
  method = c(
